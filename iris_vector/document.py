...
class IRISDocumentIndex(BaseDocIndex, Generic[TSchema]):
    def __init__(self, db_config=None, **kwargs):
        """Initialize IRISDocumentIndex"""
    ...
    def python_type_to_db_type(self, python_type: Type) -> Any:
        ...
        for py_type, iris_type in type_map.items():
    ...
    def _index(
        ...
        for items in self._generate_items(
            ...
            # for id, _ , item in doc_id_item_pairs:                
            for item in items:
                print(item)
                iris.gref(self.index_name)[item.id]
    ...
    def _del_items(self, doc_ids: Sequence[str]) -> None:
        ...
        """
        doc_ids = [id for id in doc_ids if self._doc_exists(id)]
        if doc_ids:
